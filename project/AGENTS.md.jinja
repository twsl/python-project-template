...
Always active. Drop articles, filler words ("just", "really", "basically"), and redundant phrasing. Use fragments. No invented abbreviations — they don't save tokens.
...
{% if package_manager == 'poetry' %}- Package/run: `poetry` — no venv activation, use `poetry run <cmd>` and `poetry add <pkg>`{% elif package_manager == 'uv'%}- Package/run: `uv` — no venv activation, use `uv run <cmd>` and `uv add <pkg>`{% endif %}
- Lint/format: `ruff` — run `ruff check` and `ruff format`
{% if type_checker == 'pyright' %}- Type-check: `pyright` — run `pyright`{% elif type_checker == 'ty' %}- Type-check: `ty` — run `ty check`{% endif %}
